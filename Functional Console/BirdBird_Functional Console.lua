...
reaper_do_file('functional_console_libraries/functions.lua')
reaper_do_file('functional_console_libraries/macro_library.lua')
function load_macros()
    user_macros = check_macros()
    for k,v in pairs(user_macros) do macros[k] = v end
end
load_macros()
...
function execute_command(input_command)
