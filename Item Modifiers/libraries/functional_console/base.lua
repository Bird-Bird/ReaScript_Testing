...
reaper_do_file('state.lua')
reaper_do_file('basic_commands.lua')
reaper_do_file('random.lua')
reaper_do_file('validation.lua')
reaper_do_file('item_data.lua')
...
function ext_execute(input, has_undo, clear_items)
  local success, err = fc_run(input)
  if success then
    return true, nil
  else
...
function ext_clear(clear_selection)
  fc_clear_state(clear_selection)
end
...
function ext_clear_external_selections()
